...
self.addEventListener('install', function(event) {
  console.log('[SW MoneyAI] Instalando v1.1...');
  event.waitUntil(
...
self.addEventListener('fetch', function(event) {
  ...
  //    Se reconstruye el request con redirect:'follow' para que el SW
  //    maneje correctamente los redirects de GAS hacia googleusercontent.com
  if (url.hostname === 'script.google.com' || url.hostname === 'script.googleusercontent.com') {
    event.respondWith(
      event.request.clone().text().then(function(bodyText) {
        return fetch(event.request.url, {
          method: event.request.method,
          headers: { 'Content-Type': 'application/json' },
          body: event.request.method !== 'GET' ? bodyText : undefined,
          redirect: 'follow'
        });
      }).catch(function() {
        return new Response(
          JSON.stringify({ ok: false, offline: true, msg: 'Sin conexión' }),
